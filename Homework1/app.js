...
//
// Libraries via CDN in index.html:
...
(() => {
  ...
  function mergeDatasets(trainRows, testRows) {
    const train = normalizeRows(trainRows, "train");
    const test = normalizeRows(testRows, "test");
  ...
  function renderNumericStatsTable(tableEl, stats) {
    ...
    for (const s of stats) {
      const tr = document.createElement("tr");
      const cells = [s.feature, String(s.n), roundMaybe(s.mean), roundMaybe(s.median), roundMaybe(s.std)];
      cells.forEach((c) => {
        const td = document.createElement("td");
        td.textContent = c;
        tr.appendChild(td);
  ...
  function renderRateBarChart(canvasEl, existingChartRef, data) {
    ...
    const chart = destroyChart(existingChartRef);
  ...
  function renderCorrelationBubbleMatrix(trainRows) {
    const numericCols = [SCHEMA.target, "Pclass", "Age", "SibSp", "Parch", "Fare"];
    const colsPresent = new Set(getColumns(trainRows));
    const cols = numericCols.filter((c) => colsPresent.has(c));
    ...
    const M = correlationMatrix(trainRows, cols);
    const points = [];
    ...
    state.charts.corr = new Chart(el.corrHeatmap, {
      ...
      options: {
        ...
        scales: {
          x: {
            min: -0.5, max: cols.length - 0.5,
            ticks: { callback: (v) => cols[v] ?? "", autoSkip: false, maxRotation: 0 },
            grid: { color: "rgba(255,255,255,0.06)" }
          },
          y: {
            min: -0.5, max: cols.length - 0.5,
            ticks: { callback: (v) => cols[v] ?? "", autoSkip: false },
            grid: { color: "rgba(255,255,255,0.06)" }
          }
        },
        plugins: {
  ...
  function renderGroupedNumericTable(tableEl, stats0, stats1) {
    ...
    const header = ["Feature",
      "Mean (0)", "Median (0)", "Std (0)", "N (0)",
      "Mean (1)", "Median (1)", "Std (1)", "N (1)"
    ];
    const trh = document.createElement("tr");
    header.forEach((h) => {
      const th = document.createElement("th");
    ...
    const map0 = new Map(stats0.map((s) => [s.feature, s]));
    const map1 = new Map(stats1.map((s) => [s.feature, s]));
    const features = Array.from(new Set([...map0.keys(), ...map1.keys()]));

    for (const f of features) {
      const s0 = map0.get(f) || { mean: null, median: null, std: null, n: 0 };
      const s1 = map1.get(f) || { mean: null, median: null, std: null, n: 0 };

      const tr = document.createElement("tr");
      const cells = [
        f,
        roundMaybe(s0.mean), roundMaybe(s0.median), roundMaybe(s0.std), String(s0.n),
        roundMaybe(s1.mean), roundMaybe(s1.median), roundMaybe(s1.std), String(s1.n),
      ];
      cells.forEach((c) => {
        const td = document.createElement("td");
        td.textContent = c;
